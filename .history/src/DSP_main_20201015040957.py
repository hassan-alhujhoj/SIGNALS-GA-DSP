...
class DSP_Signal():
    ...
    def IIR(self, fs):
        ...
        a = 1 * 1
        b = (1 * -np.exp(-deg1*i)) + (1*-np.exp(deg1*i))
        c = (1*-np.exp(-deg1*i)) * (1*-np.exp(deg1*i))

        d = 1 * 1
        e = (-r*np.exp(-deg1*i)) + (-r*np.exp(deg1*i))
        f = (-r*np.exp(-deg1*i)) * (-r*np.exp(deg1*i))

        g = 1 * 1
        h = (-1*np.exp(-deg2*i)) + (-1*np.exp(deg2*i))
        ii = (-1*np.exp(-deg2*i)) * (-1*np.exp(deg2*i))

        j = 1 * 1
        k = (-r*np.exp(-deg2*i)) + (-r*np.exp(deg2*i))
        l = -r*np.exp(-deg2*i) * -r*np.exp(deg2*i)

        # Calculte the gain of the overall transfer function
        Wf = 2 * np.pi * 10
        ND_array = [np.exp(0), np.exp(n[i * Wf), np.exp(-2 * Wf)]
