...
class DSP_Signal():
    ...
    def IIR(self, fs):
        ...
        d = 1 * 1;
        e = (-r*exp(-deg1*1i)) + (-r*exp(deg1*i))
        f = (-r*exp(-deg1*1i)) * (-r*exp(deg1*i))

        g = 1 * 1;
        h = (-1*exp(-deg2*1i)) + (-1*exp(deg2*i))
        ii = (-1*exp(-deg2*1i)) * (-1*exp(deg2*i))

        j = 1 * 1;
        k = (-r*exp(-deg2*1i)) + (-r*exp(deg2*i))
        l = -r*exp(-deg2*1i) * -r*exp(deg2*1)
