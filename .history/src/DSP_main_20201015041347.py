...
class DSP_Signal():
    ...
    def IIR(self, fs):
        ...
        a = 1 * 1
        b = (1 * -np.exp(-deg1 * np.pi)) + (1 * -np.exp(deg1 * i))
        c = (1 * -np.exp(-deg1 * np.pi)) * (1 * -np.exp(deg1 * i))

        d = 1 * 1
        e = (-r * np.exp(-deg1 * np.pi)) + (-r * np.exp(deg1 * i))
        f = (-r *  np.exp(-deg1 * np.pi)) * (-r * np.exp(deg1 * i))

        g = 1 * 1
        h = (-1 * np.exp(-deg2 * np.pi)) + (-1 * np.exp(deg2*i))
        ii = (-1 * np.exp(-deg2 * np.pi)) * (-1 * np.exp(deg2*i))
